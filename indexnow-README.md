...
| File | Purpose |
|------|---------|
| `indexnow-form.html` | Browser-based form for submitting a URL to IndexNow |
| `indexnow-submit.php` | PHP script that handles the actual API submission |
| `your-key-here.txt` | Example API key verification file (replace with your own) |
...
Create a plain text file named `your-key.txt` (where `your-key` is your actual key string) containing only the key string itself on a single line. Upload it to your site's root directory.

Verify it's accessible by visiting `https://yourdomain.com/your-key.txt` in a browser. You should see just the key string.

### Step 3 — Upload the PHP Script

Upload `indexnow-submit.php` to your site. It can live anywhere — root, a subdirectory, doesn't matter. On GoDaddy shared hosting this works without any special server configuration.

### Step 4 — Upload the HTML Form

Upload `indexnow-form.html` to the same directory as the PHP script. This gives you a simple browser interface for submitting URLs without touching the command line.

### Step 5 — Edit the PHP Script

Open `indexnow-submit.php` and update these three values:

```php
$api_key  = 'your-key-here';        // Your IndexNow API key
$host     = 'yourdomain.com';       // Your domain without https://
$key_location = 'https://yourdomain.com/your-key-here.txt'; // Full URL to your key file
```
...
Navigate to `indexnow-form.html` in your browser. Enter the full URL you want to submit (including `https://`) and click Submit. The PHP script posts it to the Bing IndexNow endpoint and returns the HTTP response code.
...
- This tool submits to `https://api.indexnow.org/indexnow` — the canonical IndexNow endpoint. Per the protocol specification, the receiving engine automatically shares your submission with all other participating engines within 10 seconds. One submission covers all of them.
